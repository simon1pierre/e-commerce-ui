...
import { CartItemType, CartItemTypes } from "@/types";
  ...
  return (
    <div className="flex flex-col gap-8 items-center justify-center mt-12">
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-center">
        {steps.map(step=>(
            <div className={`flex items-center gap-2 border-b-2 pb-4 ${ step.id === activeStep ? "border-gray-800" : "border-gray-200"}`} key={step.id}>
                <div className={`w-6 h-6 rounded-full text-white p-3 flex items-center justify-center ${step.id===activeStep? "bg-gray-800":"bg-gray-400"}`}>{step.id}</div>
      ...
      <div className="w-full flex flex-col lg:flex-row gap-16">
        ...
        {activeStep ===1 ? ("Products"): activeStep===2? <ShippingForm/>:activeStep===3 && shippingForm? <PaymentForm/>: <p className="text-sm text-gray-500">Please fill shipping form to continue!</p>}
        </div>
        {/* Details */}
